...
import alma.control.gui.antennamount.statuspanel.StatusRootPanel;
...
public class MountStatusPluginHolder extends PluginHolder {
	
	private final static String MOUNT_CONTROL_PLUGIN_NAME = "Mount Status";

	public MountStatusPluginHolder(
			String antennaNames[], 
		...
		plugin = new StatusRootPanel();
		plugin.setServices(svcs);
		antennas=antennaNames;
		setLayout(new BorderLayout());
		add((StatusRootPanel)plugin,BorderLayout.CENTER);
	}
	...
	public void specializedStart() throws Exception {
		...
		((StatusRootPanel)plugin).connectPerformed(sce);
	}
