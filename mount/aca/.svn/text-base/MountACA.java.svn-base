 ...
 *
 */
public final class MountACA extends Mount {
...
*/ 
		...
		alma.Control.MountACA.ACU_ERROR_DESC_00,
		alma.Control.MountACA.ACU_ERROR_DESC_01,
		alma.Control.MountACA.ACU_ERROR_DESC_02,
		alma.Control.MountACA.ACU_ERROR_DESC_03,
		alma.Control.MountACA.ACU_ERROR_DESC_04,
		alma.Control.MountACA.ACU_ERROR_DESC_05,
		alma.Control.MountACA.ACU_ERROR_DESC_06,
		// 0x07..0x0F are not assigned
		" "," "," "," "," "," "," "," "," ",
		alma.Control.MountACA.ACU_ERROR_DESC_10,
		alma.Control.MountACA.ACU_ERROR_DESC_11,
		alma.Control.MountACA.ACU_ERROR_DESC_12,
		alma.Control.MountACA.ACU_ERROR_DESC_13,
		alma.Control.MountACA.ACU_ERROR_DESC_14,
		alma.Control.MountACA.ACU_ERROR_DESC_15,
		alma.Control.MountACA.ACU_ERROR_DESC_16,
		alma.Control.MountACA.ACU_ERROR_DESC_17
	};
	 ...
	 * The ACA mount
	 */
	private final alma.Control.MountACA aca;
	 ...
	 * The subreflector
	 */
	private ACASubreflector subreflector;
	 ...
	 */
	public MountACA(
			alma.Control.MountACA mnt, 
			ACSComponentsManager.AntennaType type, 
			ContainerServices contSvcs,
			AntennaRootPane rootP) {
		super(mnt,type,contSvcs,rootP);
		if (type!=ACSComponentsManager.AntennaType.MELCO) {
			throw new IllegalStateException("The component is MountACA but the passed type is "+type);
		}
		for (Integer t=0; t<acuStates.length; t++) {
			acuStateDescriptor.put(t, acuStates[t]);
		}
		aca=mnt;
		shutter = new Shutter(aca,listenersNotifier,logger);
		subreflector = new ACASubreflector(mnt,antennaRootP,logger);
		metrology = new Metrology(aca, logger,listenersNotifier);
	...
	public void update(UpdateError errState) throws Exception {
		...
		try {
			int vals[]=aca.GET_SYSTEM_STATUS(time);
		...
		try {
			int vals[]=aca.GET_SYSTEM_STATUS_2(time);
		...
		preReadTime = System.currentTimeMillis();
		try {
			int vals[]=aca.GET_EL_STATUS(time);
		...
		try {
			int vals[]=aca.GET_EL_STATUS_2(time);
		...
		preReadTime = System.currentTimeMillis();
		try {
			int vals[]=aca.GET_AZ_STATUS(time);
		...
		try {
			int vals[]=aca.GET_AZ_STATUS_2(time);
		...
		preReadTime = System.currentTimeMillis();
		try {
			Integer status=aca.GET_SHUTTER(time);
		...
		preReadTime = System.currentTimeMillis();
		try {
			int vals[]=aca.GET_POWER_STATUS(time);
