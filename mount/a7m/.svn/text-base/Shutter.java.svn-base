...
public class Shutter extends ShutterCommon {
	 ...
	 */
	public Shutter(MountA7M a7m, MountListenersNotifier notifier,Logger logger) {
		...
		Thread t = new Thread(new Runnable() {
			public void run() {
				...
				if (newMode==null) {
					...
					ex.setAntennatype(AntennaType.MELCOA7M.description);
					listenersNotifier.commandExecuted(0,"Unsupported A7M shutter to,"+action.toString(),"Unsupported A7M shutter to,"+action.toString(),ex);
					return;
				}
				try {
					logger.log(AcsLogLevel.DEBUG,"Setting A7M shutter to "+newMode);
					mount.SET_SHUTTER(newMode);
					logger.log(AcsLogLevel.DEBUG,"Setting A7M shutter to "+newMode+" done");
				} catch (Throwable t) {
					AcsJShutterEx ex = new AcsJShutterEx(t);
					ex.setOperation("Error setting the shutter to "+action.toString());
					ex.setAntennatype(AntennaType.MELCOA7M.description);
					listenersNotifier.commandExecuted(0,"Set A7M shutter to,"+newMode,"Error from remote component while setting shutter to "+action.toString(),ex);
				}		
			}
		});
		t.setDaemon(true);
		t.setName("A7M:setShutter");
		t.start();
	}
