...
/**
 * 
 * The class to show details of the detailed status of the ACA production
 *
 */
public class MountACAStatus extends MountDetails implements MountInterface {
	
	/**
	 * The titles of the rows of the status of the ACA antenna.
	...
	private enum SysStatus {
		...
		IF_PANEL_FAIL("Interface panel failure (F)"), 
			// bit2: reserved 
		NUTATOR_RACK_PS_ALARM("Nutator rack power supply alarm (F)"), 
	...
	/**
	 *  The titles of the rows of the status of the ACA elevation.
	 *  
	...
	/**
	 *  The titles of the rows of the status of the ACA Azimuth.
	 *  
	 *  The order of declaration in this enum reflect the order in the table
	 *  
	 *  @author acaproni
	 ...
	 * @param mnt The mount
	 */
	private ValueState updateSystemStatus(MountACA mnt) {
		if (mnt==null) {
			updateError();
	...
	private ValueState updateSystemStatus2(MountACA mnt) {
		if (mnt==null) {
			updateError();
			return ValueState.NORMAL;
		}
	 ...
	 * @param mnt The mount
	 */
	private ValueState updatePowerStatus(MountACA mnt) {
		if (mnt==null) {
			updateError();
			return ValueState.NORMAL;
		}
	...
	public ValueState refreshValues(Mount mnt) {
		if (!(mnt instanceof MountACA)) {
			throw new IllegalArgumentException("The mount is not MountACA");
		}
		ValueState st=updateSystemStatus((MountACA)mnt);
		st=ValueState.max(st,updateSystemStatus2((MountACA)mnt));
		st=ValueState.max(st,updateAzStatus((MountACA)mnt));
		st=ValueState.max(st,updateAzStatus2((MountACA)mnt));
		st=ValueState.max(st,updateElStatus((MountACA)mnt));
		st=ValueState.max(st,updateElStatus2((MountACA)mnt));
		st=ValueState.max(st,updatePowerStatus((MountACA)mnt));
		return st;
	}
	 ...
	 * @param mnt The mount
	 */
	private ValueState updateAzStatus(MountACA mnt) {
		if (mnt==null) {
			updateError();
			return ValueState.NORMAL;
		}
	...
	private ValueState updateAzStatus2(MountACA mnt) {
		if (mnt==null) {
			updateError();
			return ValueState.NORMAL;
		}
	 ...
	 * @param mnt The mount
	 */
	private ValueState updateElStatus(MountACA mnt) {
		if (mnt==null) {
			updateError();
			return ValueState.NORMAL;
		}
	...
	private ValueState updateElStatus2(MountACA mnt) {
		if (mnt==null) {
			updateError();
			return ValueState.NORMAL;
		}
